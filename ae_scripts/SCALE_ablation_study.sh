...
#SBATCH --job-name=SCALE_ablation_study   # Job name
...
###################################################################################################
# 98 1200 1200
output_file="./ae_results/SCALE_Results/SCALE_${variable}_ablation.txt"
tmp_file="./ae_results/SCALE_Results/SCALE_${variable}_tmp.txt"
...
# Baselines
# 1. IPComp
./external/IPComp/build/src/refactor $data_file "-d" "-3" 98 1200 1200 $refactor_file > $tmp_file
time=$(grep "time" $tmp_file | head -n 1)
echo "Method#1, IPComp, Refactor time: $time" >> $output_file
for error_bound in "${error_bounds[@]}"; do
    ./external/IPComp/build/src/reconstructor $data_file "-d" "-3" 98 1200 1200 "-1" $error_bound $refactor_file > $tmp_file
...
# 2. PMGARD eb mode
./build/test/test_mdr_refactor $data_file 4 60 3 98 1200 1200 $refactor_file 1 > $tmp_file
time=$(grep "time" $tmp_file | head -n 1)
...
# 3. PMGARD psnr mode
./build/test/test_mdr_refactor $data_file 4 60 3 98 1200 1200 $refactor_file 0 > $tmp_file
time=$(grep "time" $tmp_file | head -n 1)
...
# 4. SZ3-R
./build/test/test_PSZ3-delta_refactor $data_file $refactor_file 18 3 98 1200 1200 "-d" > $tmp_file
time=$(grep "time" $tmp_file | head -n 1)
