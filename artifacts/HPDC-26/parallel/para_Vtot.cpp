...
template<class T>
bool halving_error_V_TOT_uniform(const T * Vx, const T * Vy, const T * Vz, size_t n, const double tau, std::vector<double>& ebs){
	double eb_Vx = ebs[0];
	double eb_Vy = ebs[1];
	double eb_Vz = ebs[2];
	double max_value = 0;
...
template<class T>
bool halving_error_V_TOT_coordinate(const T * Vx, const T * Vy, const T * Vz, size_t n, const double tau, std::vector<double>& ebs){
	double eb_Vx = ebs[0];
	double eb_Vy = ebs[1];
	double eb_Vz = ebs[2];
	double max_value = 0;
...
template<class T>
bool halving_error_V_TOT_uniform(const T * Vx, const T * Vy, const T * Vz, size_t n, const double tau, std::vector<double>& ebs, std::vector<std::vector<int>> weights){
	double eb_Vx = ebs[0];
	double eb_Vy = ebs[1];
	double eb_Vz = ebs[2];
	double max_value = 0;
...
template<class T>
bool halving_error_V_TOT_coordinate(const T * Vx, const T * Vy, const T * Vz, size_t n, const double tau, std::vector<double>& ebs, std::vector<std::vector<int>> weights){
	double eb_Vx = ebs[0];
	double eb_Vy = ebs[1];
	double eb_Vz = ebs[2];
	double max_value = 0;
...
std::vector<size_t> retrieve_V_TOT_HPEZ(std::string rdata_file_prefix, double tau, std::vector<double> ebs, size_t num_elements, int weighted, double & max_act_error, double & max_est_error, size_t & weight_file_size, bool decrease_method){
	...
	if(!weighted){
        ...
        local_elapsed_time += MPI_Wtime();
		hpez_reconstructors = reconstructors;
	}
	return total_retrieved_size;
}
...
int main(int argc, char ** argv){
	...
	case GE:
		...
		break;
	case HPEZ:
		total_retrieved_size = retrieve_V_TOT_HPEZ<T>(rdata_file_prefix, tau, ebs, num_elements, weighted, local_max_act_error, local_max_est_error, weight_file_size, decrease_method);
		break;
	default:
		break;
	}
