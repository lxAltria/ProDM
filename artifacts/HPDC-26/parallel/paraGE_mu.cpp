...
template<class T>
bool halving_error_mu_uniform(const T * P, const T * D, size_t n, const T tau, std::vector<T>& ebs){
	T eb_P = ebs[0];
	T eb_D = ebs[1];
	T R = 287.1;
	T gamma = 1.4;
...
template<class T>
bool halving_error_mu_coordinate(const T * P, const T * D, size_t n, const T tau, std::vector<T>& ebs){
	T eb_P = ebs[0];
	T eb_D = ebs[1];
	T R = 287.1;
	T gamma = 1.4;
...
template<class T>
bool halving_error_mu_uniform(const T * P, const T * D, size_t n, const T tau, std::vector<T>& ebs, std::vector<std::vector<int>> weights){
	T eb_P = ebs[0];
	T eb_D = ebs[1];
	T R = 287.1;
	T gamma = 1.4;
...
template<class T>
bool halving_error_mu_coordinate(const T * P, const T * D, size_t n, const T tau, std::vector<T>& ebs, std::vector<std::vector<int>> weights){
	T eb_P = ebs[0];
	T eb_D = ebs[1];
	T R = 287.1;
	T gamma = 1.4;
...
std::vector<size_t> retrieve_mu_Dummy(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size, bool decrease_method){
    ...
    if(!weighted){
        ...
        while((!tolerance_met) && (iter < max_iter)){
            ...
            ProDM::print_vec(ebs);
            tolerance_met = halving_error_mu_uniform(P_dec, D_dec, num_elements, tau, ebs);
        ...
        while((!tolerance_met) && (iter < max_iter)){
            ...
            std::cout << "iter" << iter << ": The old ebs are:" << std::endl;
            ProDM::print_vec(ebs);
            tolerance_met = halving_error_mu_uniform(P_dec, D_dec, num_elements, tau, ebs, weights);
            std::cout << "iter" << iter << ": The new ebs are:" << std::endl;
            ProDM::print_vec(ebs);
            /* test
...
std::vector<size_t> retrieve_mu_SZ3(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size, bool decrease_method){
    ...
    if(!weighted){
        ...
        while((!tolerance_met) && (iter < max_iter)){
            ...
            // std::cout << "iter" << iter << ": The old ebs are:" << std::endl;
            // ProDM::print_vec(ebs);
            if(!decrease_method) tolerance_met = halving_error_mu_uniform(P_dec, D_dec, num_elements, tau, ebs);
            else tolerance_met = halving_error_mu_coordinate(P_dec, D_dec, num_elements, tau, ebs);
            // std::cout << "iter" << iter << ": The new ebs are:" << std::endl;
            // ProDM::print_vec(ebs);
            // std::cout << names[1] << " requested error = " << tau << std::endl;
            max_act_error = print_max_abs(names[1] + " error", error_mu);
        ...
        while((!tolerance_met) && (iter < max_iter)){
            ...
            // std::cout << "iter" << iter << ": The old ebs are:" << std::endl;
            // ProDM::print_vec(ebs);
            if(!decrease_method) tolerance_met = halving_error_mu_uniform(P_dec, D_dec, num_elements, tau, ebs, weights);
            else tolerance_met = halving_error_mu_coordinate(P_dec, D_dec, num_elements, tau, ebs, weights);
            // std::cout << "iter" << iter << ": The new ebs are:" << std::endl;
            // ProDM::print_vec(ebs);
            /* test
            std::string filename = "./Result/Temp_err.dat";
...
int main(int argc, char ** argv){
    ...
    {
    case Dummy:
        total_retrieved_size = retrieve_mu_Dummy<T>(rdata_file_prefix, tau, ebs, num_elements, weighted, local_max_act_error, local_max_est_error, weight_file_size, decrease_method);
        break;
    case SZ3:
        total_retrieved_size = retrieve_mu_SZ3<T>(rdata_file_prefix, tau, ebs, num_elements, weighted, local_max_act_error, local_max_est_error, weight_file_size, decrease_method);
        break;
    case PMGARD:
        total_retrieved_size = retrieve_mu_PMGARD<T>(rdata_file_prefix, tau, ebs, num_elements, weighted, local_max_act_error, local_max_est_error, weight_file_size, decrease_method);
        break;
    case GE:
        total_retrieved_size = retrieve_mu_GE<T>(rdata_file_prefix, tau, ebs, num_elements, weighted, local_max_act_error, local_max_est_error, weight_file_size, decrease_method);
        break;
    default:
        break;
    }
