...
#SBATCH --job-name=Miranda_ablation_study   # Job name
...
###################################################################################################
# 256 384 384
output_file="./ae_results/Miranda_Results/Miranda_${variable}_ablation.txt"
tmp_file="./ae_results/Miranda_Results/Miranda_${variable}_tmp.txt"
...
# Baselines
# 1. IPComp
./external/IPComp/build/src/refactor $data_file "-d" "-3" 256 384 384 $refactor_file > $tmp_file
time=$(grep "time" $tmp_file | head -n 1)
echo "Method#1, IPComp, Refactor time: $time" >> $output_file
for error_bound in "${error_bounds[@]}"; do
    ./external/IPComp/build/src/reconstructor $data_file "-d" "-3" 256 384 384 "-1" $error_bound $refactor_file > $tmp_file
...
# 2. PMGARD eb mode
./build/test/test_mdr_refactor $data_file 4 60 3 256 384 384 $refactor_file 1 > $tmp_file
time=$(grep "time" $tmp_file | head -n 1)
...
# 3. PMGARD psnr mode
./build/test/test_mdr_refactor $data_file 4 60 3 256 384 384 $refactor_file 0 > $tmp_file
time=$(grep "time" $tmp_file | head -n 1)
...
# 4. SZ3-R
./build/test/test_PSZ3-delta_refactor $data_file $refactor_file 18 3 256 384 384 "-d" > $tmp_file
time=$(grep "time" $tmp_file | head -n 1)
...
./build/test/test_proaicd_refactor $data_file "-d" 4 60 3 256 384 384 $refactor_file "-PerBit" "-eb" "-no_CP" > $tmp_file
...
./build/test/test_proaicd_refactor $data_file "-d" 4 60 3 256 384 384 $refactor_file "-PerBit" "-eb" "-CP" > $tmp_file
time=$(grep "Refactor_time" $tmp_file | head -n 1)
...
./build/test/test_proaicd_refactor $data_file "-d" 4 60 3 256 384 384 $refactor_file "-XOR" "-PSNR" "-no_CP" > $tmp_file
time=$(grep "Refactor_time" $tmp_file | head -n 1)
...
./build/test/test_proaicd_refactor $data_file "-d" 4 60 3 256 384 384 $refactor_file "-XOR" "-PSNR" "-CP" > $tmp_file
time=$(grep "Refactor_time" $tmp_file | head -n 1)
