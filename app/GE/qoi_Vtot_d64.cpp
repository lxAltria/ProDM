...
bool halving_error_V_TOT_coordinate(const T * Vx, const T * Vy, const T * Vz, size_t n, const std::vector<unsigned char>& mask, const T tau, std::vector<T>& ebs, std::vector<std::vector<int>> weights){
	...
	// std::cout << names[0] << ": max estimated error = " << max_value << ", index = " << max_index << ", e_V_TOT_2 = " << max_e_V_TOT_2 << ", VTOT_2 = " << max_V_TOT_2 << ", Vx = " << max_Vx << ", Vy = " << max_Vy << ", Vz = " << max_Vz << std::endl;
	// std::cout << "eb_Vx = " << ldexp(eb_Vx, -weights[0][max_weight_index]) << ", eb_Vy = " << ldexp(eb_Vy, -weights[1][max_weight_index]) << ", eb_Vz = " << ldexp(eb_Vz, -weights[2][max_weight_index]) << std::endl;
	// std::cout << "max_weight_Vx = " << weights[0][max_weight_index] << ", max_weight_Vy = " << weights[1][max_weight_index] << ", max_weight_Vz = " << weights[2][max_weight_index] << std::endl;
...
template<class T>
std::vector<size_t> retrieve_V_TOT_HPEZ_update_eb(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, const std::vector<unsigned char>& mask, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size, bool decrease_method){
    int max_iter = 30;
	bool tolerance_met = false;
	int n_variable = ebs.size();
	std::vector<std::vector<T>> reconstructed_vars(n_variable, std::vector<T>(num_elements));
	...
	if(!weighted){
		...
		while((!tolerance_met) && (iter < max_iter)){
			...
			for(int i=0; i<n_variable; i++){
				...
				ebs[i] = reconstructors[i].get_current_eb(); //
			...
			error_V_TOT = std::vector<T>(num_elements);
			error_est_V_TOT = std::vector<T>(num_elements);
			// std::cout << "iter" << iter << ": The current ebs are:" << std::endl;
			// ProDM::print_vec(ebs);
...
std::vector<size_t> retrieve_V_TOT_SZ2(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, const std::vector<unsigned char>& mask, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size, bool decrease_method){
	...
	std::vector<size_t> total_retrieved_size(n_variable, 0);
	if(!weighted){
		std::vector<PDR::ApproximationBasedReconstructor<T, PDR::SZ2Approximator<T>, ProDM::NegaBinaryBPEncoder<T, T_stream>, AdaptiveLevelCompressor, SignExcludeGreedyBasedSizeInterpreter<ProDM::MaxErrorEstimatorHB<T>>, MaxErrorEstimatorHB<T>, ConcatLevelFileRetriever>> reconstructors;
		for(int i=0; i<n_variable; i++){
			std::string rdir_prefix = rdata_file_prefix + varlist[i];
			std::string metadata_file = rdir_prefix + "_refactored/metadata.bin";
			std::vector<std::string> files;
		...
		std::vector<PDR::WeightedApproximationBasedReconstructor<T, PDR::SZ2Approximator<T>, ProDM::WeightedNegaBinaryBPEncoder<T, T_stream>, AdaptiveLevelCompressor, SignExcludeGreedyBasedSizeInterpreter<ProDM::MaxErrorEstimatorHB<T>>, MaxErrorEstimatorHB<T>, ConcatLevelFileRetriever>> reconstructors;
		std::vector<std::vector<int>> weights(n_variable, std::vector<int>(num_elements));
		for(int i=0; i<n_variable; i++){
			std::string rdir_prefix = rdata_file_prefix + varlist[i];
			std::string metadata_file = rdir_prefix + "_refactored/metadata.bin";
...
std::vector<size_t> retrieve_V_TOT_MGARD(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, const std::vector<unsigned char>& mask, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size, bool decrease_method){
	...
	std::vector<size_t> total_retrieved_size(n_variable, 0);
	if(!weighted){
		std::vector<PDR::ApproximationBasedReconstructor<T, PDR::MGARDApproximator<T>, ProDM::NegaBinaryBPEncoder<T, T_stream>, AdaptiveLevelCompressor, SignExcludeGreedyBasedSizeInterpreter<ProDM::MaxErrorEstimatorHB<T>>, MaxErrorEstimatorHB<T>, ConcatLevelFileRetriever>> reconstructors;
		for(int i=0; i<n_variable; i++){
			std::string rdir_prefix = rdata_file_prefix + varlist[i];
			std::string metadata_file = rdir_prefix + "_refactored/metadata.bin";
			std::vector<std::string> files;
		...
		std::vector<PDR::WeightedApproximationBasedReconstructor<T, PDR::MGARDApproximator<T>, ProDM::WeightedNegaBinaryBPEncoder<T, T_stream>, AdaptiveLevelCompressor, SignExcludeGreedyBasedSizeInterpreter<ProDM::MaxErrorEstimatorHB<T>>, MaxErrorEstimatorHB<T>, ConcatLevelFileRetriever>> reconstructors;
		std::vector<std::vector<int>> weights(n_variable, std::vector<int>(num_elements));
		for(int i=0; i<n_variable; i++){
			std::string rdir_prefix = rdata_file_prefix + varlist[i];
			std::string metadata_file = rdir_prefix + "_refactored/metadata.bin";
	...
	if(!weighted){
		...
		while((!tolerance_met) && (iter < max_iter)){
			...
			for(int i=0; i<n_variable; i++){
				...
				// reconstructor_timer.start();
				auto reconstructed_data = reconstructors[i].progressive_reconstruct(ldexp(ebs[i], -reconstructors[i].get_max_weight()), -1);
				// std::cout << "Var#" << i << ", requested eb = " << ebs[i] << ", current eb = " << reconstructors[i].get_current_eb(reconstructors[i].get_max_weight()) << std::endl;
				ebs[i] = reconstructors[i].get_current_eb();
				// reconstructor_timer.end();
				// reconstructor_timer.print("Reconstructor");
				total_retrieved_size[i] = reconstructors[i].get_retrieved_size();
...
int main(int argc, char ** argv){
	...
	// 	std::cout << "Var#" << i << "'s value range = " << var_range[i] << std::endl;
    // } 
