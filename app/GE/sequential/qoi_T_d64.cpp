...
template<class T>
bool halving_error_T_uniform(const T * P, const T * D, size_t n, const T tau, std::vector<T>& ebs){
	T eb_P = ebs[0];
	T eb_D = ebs[1];
	T R = 287.1;
	T c_1 = 1.0 / R;
...
template<class T>
bool halving_error_T_coordinate(const T * P, const T * D, size_t n, const T tau, std::vector<T>& ebs){
	T eb_P = ebs[0];
	T eb_D = ebs[1];
	T R = 287.1;
	T c_1 = 1.0 / R;
...
template<class T>
bool halving_error_T_uniform(const T * P, const T * D, size_t n, const T tau, std::vector<T>& ebs, std::vector<std::vector<int>> weights){
	T eb_P = ebs[0];
	T eb_D = ebs[1];
	T R = 287.1;
	T c_1 = 1.0 / R;
	...
	if(max_value > tau){
        ...
        while(estimate_error > tau){
            ...
            eb_P = eb_P / 1.5;
            eb_D = eb_D / 1.5;
            estimate_error = c_1 * compute_bound_division(P[i], D[i], ldexp(eb_P, -weights[0][i]), ldexp(eb_D, -weights[1][i]));
        }
        ebs[0] = eb_P;
        ebs[1] = eb_D;
		return false;
...
template<class T>
bool halving_error_T_coordinate(const T * P, const T * D, size_t n, const T tau, std::vector<T>& ebs, std::vector<std::vector<int>> weights){
	T eb_P = ebs[0];
	T eb_D = ebs[1];
	T R = 287.1;
	T c_1 = 1.0 / R;
    ...
    if(max_value > tau){
        ...
        while(estimate_error > tau){
            ...
            {
                T eb_P_ = eb_P / 1.5;
                estimate_error_P = c_1 * compute_bound_division(P[i], D[i], ldexp(eb_P_, -weights[0][i]), ldexp(eb_D, -weights[1][i]));
            }
            T estimate_error_D = 0;
            {
                T eb_D_ = eb_D / 1.5;
                estimate_error_D = c_1 * compute_bound_division(P[i], D[i], ldexp(eb_P, -weights[0][i]), ldexp(eb_D_, -weights[1][i]));
            }
            // std::cout << estimate_error_P << " " << estimate_error_D << std::endl;
...
std::vector<size_t> retrieve_T_Dummy(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size, bool decrease_method){
    ...
    if(!weighted){
        ...
        while((!tolerance_met) && (iter < max_iter)){
            ...
            ProDM::print_vec(ebs);
            tolerance_met = halving_error_T_uniform(P_dec, D_dec, num_elements, tau, ebs);
        ...
        while((!tolerance_met) && (iter < max_iter)){
            ...
            std::cout << "iter" << iter << ": The old ebs are:" << std::endl;
            ProDM::print_vec(ebs);
            tolerance_met = halving_error_T_uniform(P_dec, D_dec, num_elements, tau, ebs, weights);
            std::cout << "iter" << iter << ": The new ebs are:" << std::endl;
            ProDM::print_vec(ebs);
            /* test
...
std::vector<size_t> retrieve_T_SZ3(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size, bool decrease_method){
    ...
    if(!weighted){
        ...
        while((!tolerance_met) && (iter < max_iter)){
            ...
            // std::cout << "iter" << iter << ": The old ebs are:" << std::endl;
            // ProDM::print_vec(ebs);
            if(!decrease_method) tolerance_met = halving_error_T_uniform(P_dec, D_dec, num_elements, tau, ebs);
            else tolerance_met = halving_error_T_coordinate(P_dec, D_dec, num_elements, tau, ebs);
            // std::cout << "iter" << iter << ": The new ebs are:" << std::endl;
            // ProDM::print_vec(ebs);
            // std::cout << names[1] << " requested error = " << tau << std::endl;
            max_act_error = print_max_abs(names[1] + " error", error_Temp);
        ...
        while((!tolerance_met) && (iter < max_iter)){
            ...
            // std::cout << "iter" << iter << ": The old ebs are:" << std::endl;
            // ProDM::print_vec(ebs);
            if(!decrease_method) tolerance_met = halving_error_T_uniform(P_dec, D_dec, num_elements, tau, ebs, weights);
            else tolerance_met = halving_error_T_coordinate(P_dec, D_dec, num_elements, tau, ebs, weights);
            // std::cout << "iter" << iter << ": The new ebs are:" << std::endl;
            // ProDM::print_vec(ebs);
            /* test
            std::string filename = "./Result/Temp_err.dat";
...
int main(int argc, char ** argv){
    ...
    {
    case Dummy_Cmp:
        total_retrieved_size = retrieve_T_Dummy<T>(rdata_file_prefix, tau, ebs, num_elements, weighted, max_act_error, max_est_error, weight_file_size, decrease_method);
        break;
    case SZ3_Cmp:
        total_retrieved_size = retrieve_T_SZ3<T>(rdata_file_prefix, tau, ebs, num_elements, weighted, max_act_error, max_est_error, weight_file_size, decrease_method);
        break;
    case PMGARD:
        total_retrieved_size = retrieve_T_PMGARD<T>(rdata_file_prefix, tau, ebs, num_elements, weighted, max_act_error, max_est_error, weight_file_size, decrease_method);
        break;
    case GE_Cmp:
        total_retrieved_size = retrieve_T_GE<T>(rdata_file_prefix, tau, ebs, num_elements, weighted, max_act_error, max_est_error, weight_file_size, decrease_method);
        break;
    default:
        break;
    }
