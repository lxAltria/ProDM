...
namespace MDR {
    ...
    template<class T>
    class MaxErrorEstimatorHBCubic_hybrid : public MaxErrorEstimator<T> {
    public:
        MaxErrorEstimatorHBCubic_hybrid(int num_dims){
            switch(num_dims){
                case 1:
                    c = 5.0/4;
                    break;
        ...
        MaxErrorEstimatorHBCubic_hybrid() : MaxErrorEstimatorHBCubic_hybrid(1) {}
        inline T estimate_error(T error, int level) const {
            return c*error;
        }
        inline T estimate_error(T error, int level, int num_levels) const {
