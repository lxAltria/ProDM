...
namespace MDR {
    ...
    public:
        WeightedNegaBinaryBPEncoder(){
            static_assert(std::is_floating_point<T_data>::value, "WeightedNegaBinaryBPEncoder: input data must be floating points.");
            static_assert(!std::is_same<T_data, long double>::value, "WeightedNegaBinaryBPEncoder: long double is not supported.");
            static_assert(std::is_unsigned<T_stream>::value, "WeightedNegaBinaryBPEncoder: streams must be unsigned integers.");
            static_assert(std::is_integral<T_stream>::value, "WeightedNegaBinaryBPEncoder: streams must be unsigned integers.");
        }
        ...
        std::vector<uint8_t *> encode(T_data const * data, int32_t n, int32_t exp, uint8_t num_bitplanes, std::vector<uint32_t>& stream_sizes, std::vector<double>& level_errors, int * weights = NULL) const {
        ...
        void set_num_weight_bitplane(int num){
            weight_bitplane = num;
        }
