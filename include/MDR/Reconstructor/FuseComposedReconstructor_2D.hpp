...
namespace MDR {
    ...
    class FuseComposedReconstructor_2D : public concepts::ReconstructorInterface<T> {
    public:
        FuseComposedReconstructor_2D(Decomposer decomposer, Interleaver interleaver, Encoder encoder, Compressor compressor, SizeInterpreter interpreter, Retriever retriever)
        ...
        void load_metadata(){
            ...
            // PSZ levels
            // MGARD_target_level = (level_error_bounds.size() - 1) / 2;
            // std::cout << "MGARD_target_level = " << MGARD_target_level << std::endl;
            // std::vector<uint32_t> dim_uint32;
            // for(int i=0; i<dimensions.size(); i++){
            //     if(i != direction) dim_uint32.push_back(dimensions[i]);
            // }
            // auto level_dims = compute_level_dims_new(dim_uint32, MGARD_target_level);
            ...
            // MGARD levels
            MGARD_target_level = level_error_bounds.size() - 1;
            level_buffer_sizes.resize(MGARD_target_level + 1, 0);
            std::vector<uint32_t> dim_uint32;
            for(int i=0; i<dimensions.size(); i++){
                if(i != direction) dim_uint32.push_back(dimensions[i]);
            }
            auto level_dims = compute_level_dims_new(dim_uint32, MGARD_target_level);
            auto level_buffer_sizes_ = compute_level_buffers_size(level_dims, MGARD_target_level);
            for(int i=0; i<level_buffer_sizes_.size(); i++){
                level_buffer_sizes_[i] *= dimensions[direction];
            }
            level_buffer_sizes[0] = level_buffer_sizes_[0];
            for(int i=1; i<=MGARD_target_level; i++){
                for(int j=1; j<=dim_uint32.size(); j++){
                    level_buffer_sizes[i] += level_buffer_sizes_[(i - 1) * 2 + j];
                }
            }
    ...
    private:
        bool reconstruct(uint8_t target_level, const std::vector<uint8_t>& prev_level_num_bitplanes, bool progressive=true){
            ...
            if(current_level >= 0){
                if(current_level) data = decomposer.reposition_recompose_split_levels(level_buffers, current_dimensions, MGARD_target_level, this->strides);
            ...
            if(current_level >= 0){
                ...
                data = decomposer.reposition_recompose_split_levels(level_buffers, reconstruct_dimensions, MGARD_target_level, this->strides);
            }
