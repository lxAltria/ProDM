...
void refactor_velocities_1D(const std::string data_file_prefix, const std::string rdata_file_prefix){
    ...
    std::vector<std::vector<Type>> Vtot(n_variable, std::vector<Type>(num_valid_data)); // test for mask in weight
    int weight_index = 0; // test for mask in weight
    for(int i=0; i<num_elements; i++){
        if(mask[i]){
            Vtot[0][weight_index] = velocityX_vec[i] / std::sqrt(velocityX_vec[i]*velocityX_vec[i] + velocityY_vec[i]*velocityY_vec[i] + velocityZ_vec[i]*velocityZ_vec[i]);
            Vtot[1][weight_index] = velocityY_vec[i] / std::sqrt(velocityX_vec[i]*velocityX_vec[i] + velocityY_vec[i]*velocityY_vec[i] + velocityZ_vec[i]*velocityZ_vec[i]);
            Vtot[2][weight_index] = velocityZ_vec[i] / std::sqrt(velocityX_vec[i]*velocityX_vec[i] + velocityY_vec[i]*velocityY_vec[i] + velocityZ_vec[i]*velocityZ_vec[i]);
            // Vtot[0][weight_index] = std::sqrt(velocityX_vec[i]*velocityX_vec[i] + velocityY_vec[i]*velocityY_vec[i] + velocityZ_vec[i]*velocityZ_vec[i]);
            // Vtot[1][weight_index] = std::sqrt(velocityX_vec[i]*velocityX_vec[i] + velocityY_vec[i]*velocityY_vec[i] + velocityZ_vec[i]*velocityZ_vec[i]);
            // Vtot[2][weight_index] = std::sqrt(velocityX_vec[i]*velocityX_vec[i] + velocityY_vec[i]*velocityY_vec[i] + velocityZ_vec[i]*velocityZ_vec[i]);
            weight_index++;
        }
    }
    std::cout << "num_elements = " << num_elements << ", num_valid_data = " << num_valid_data << std::endl;
    ...
    for(int i=0; i<n_variable; i++){
        ...
        std::cout << "index = " << index << std::endl;
        refactor.refactor(buffer.data(), dims_masked, target_level, num_bitplanes, 16);            
    }
}
