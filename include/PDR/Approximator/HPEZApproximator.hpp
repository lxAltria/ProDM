    ...
    public:
        ...
        size_t refactor_approximate(T * data, const std::vector<uint32_t>& dimensions, T approximator_eb, std::string filename=std::string(""), std::vector<uint32_t> strides=std::vector<uint32_t>()) {
            ...
            if(dimensions.size() == 1){
                ...
                QoZ::Config conf(dimensions[0], dimensions[1]);
                conf.QoZ = 4;
                conf.errorBoundMode = QoZ::EB_ABS;
                conf.absErrorBound = approximator_eb;
                cmpData = SZ_compress<T>(conf, data, cmpSize);
                ...
                QoZ::Config conf(dimensions[0], dimensions[1], dimensions[2]);
                conf.QoZ = 4;
                conf.errorBoundMode = QoZ::EB_ABS;
                conf.absErrorBound = approximator_eb;
                cmpData = SZ_compress<T>(conf, data, cmpSize);
                ...
                std::cout << "Dimension larger than 4 is not supported at HPEZ approximator!" << std::endl;
                exit(-1);
            }
        ...
        void print() const {
            std::cout << "PDR HPEZ approximator" << std::endl;
        }
    private:
