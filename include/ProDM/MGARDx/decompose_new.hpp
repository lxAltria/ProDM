#ifndef _MGARD_DECOMPOSE_NEW_HPP
#define _MGARD_DECOMPOSE_NEW_HPP
...
template <class T>
class Decomposer_new{
public:
	Decomposer_new(bool use_sz_=true){
            use_sz = use_sz_;
        };
	~Decomposer_new(){
...
private:
	...
	void compute_interpolant_difference_1D(const size_t begin, const size_t end, const size_t stride, T * buffer){
    ...
    void decompose_level_1D_with_hierarchical_basis(T * data_pos, size_t n, T h, bool nodal_row=true){
        ...
        // T * nodal_buffer = data_buffer;
        // T * coeff_buffer = data_buffer + n_nodal;
		compute_interpolant_difference_1D(0, n-1, h, data_pos);
        // data_reorder_1D(data_pos, n_nodal, n_coeff, nodal_buffer, coeff_buffer);
		// memcpy(data_pos, data_buffer, n*sizeof(T));
    }
	...
	void compute_interpolant_difference_2D(T * data_pos, size_t n1, size_t n2, size_t h, size_t stride){
		...
		for(size_t i=0; i<n1; i+=h){
			compute_interpolant_difference_1D(n2_begin, n2_end, stride_n2, cur_data_pos);
			cur_data_pos += n2 * h;
		}
		// compute vertical difference
		...
		for(size_t i=0; i<n2; i+=h){
			compute_interpolant_difference_1D(n1_begin, n1_end, stride_n1, cur_data_pos);
			cur_data_pos += h;
		}
	}
	...
	void decompose_level_2D(T * data_pos, size_t n1, size_t n2, T h, size_t stride){
        ...
        size_t n2_nodal = (n2 >> 1) + 1;
        size_t n2_coeff = n2 - n2_nodal;
		data_reorder_2D_new(data_pos, data_buffer, n1, n2, stride);
    ...
    void decompose_level_2D_with_hierarchical_basis(T * data_pos, size_t n1, size_t n2, T h, size_t stride){
        compute_interpolant_difference_2D(data_pos, n1, n2, h, stride);
		// data_reorder_2D_new(data_pos, data_buffer, n1, n2, stride);
    }
	...
	*/
	...
	void compute_interpolant_difference_3D(T * data_pos, size_t n1, size_t n2, size_t n3, size_t h, size_t dim0_stride, size_t dim1_stride){
		...
		for(size_t i=0; i<n1; i+=h){
			...
			for(size_t j=0; j<n2; j+=h){
				compute_interpolant_difference_1D(n3_begin, n3_end, h, temp_data_pos);
				temp_data_pos += n3*h;
			}
		...
		for(size_t i=0; i<n1; i+=h){
			...
			for(size_t j=0; j<n3; j+=h2x){
				compute_interpolant_difference_1D(n2_begin, n2_end, stride_n2, temp_data_pos);
				temp_data_pos += h2x;
			}
			cur_data_pos += n2*n3*h;
		}
		...
		for(size_t i=0; i<n2; i+=h2x){
			temp_data_pos = cur_data_pos;
			for(size_t j=0; j<n3; j+=h2x){
				compute_interpolant_difference_1D(n1_begin, n1_end, stride_n1, temp_data_pos);
				temp_data_pos += h2x;
			}
			cur_data_pos += n3 * h2x;
		}
	...
	void decompose_level_3D(T * data_pos, size_t n1, size_t n2, size_t n3, T h, size_t dim0_stride, size_t dim1_stride){
		data_reorder_3D_new(data_pos, data_buffer, n1, n2, n3, dim0_stride, dim1_stride);
    ...
    void decompose_level_3D_with_hierarchical_basis(T * data_pos, size_t n1, size_t n2, size_t n3, T h, size_t dim0_stride, size_t dim1_stride){
        compute_interpolant_difference_3D(data_pos, n1, n2, n3, h, dim0_stride, dim1_stride);
		// data_reorder_3D_new(data_pos, data_buffer, n1, n2, n3, dim0_stride, dim1_stride);
    }
