...
// they were written for: per-primitive bounds (x^2, sqrt, 1/(x+a), x/y, x*y) and the six GE QoIs. Kept to reproduce
// the SC'24 and HPDC'26 experiments and as the hand-bound baseline of later work. Where a bound's precondition fails (a divisor interval containing zero), the forward bounds return
// +infinity: the error is not controllable at that data bound, so the retrieval must tighten it. (The original
// implementation returned 0 here, silently accepting the point.)
