...
namespace MDR {
    ...
    class PartialOrderedCPRefactor : public concepts::RefactorInterface<T> {
    public:
        PartialOrderedCPRefactor(Decomposer decomposer, Interleaver interleaver, Encoder encoder, Compressor compressor, ErrorCollector collector, Writer writer)
        ...
        void refactor(T const * data_, const std::vector<uint32_t>& dims, uint8_t target_level, uint8_t num_bitplanes){
            ...
            combined_coeff_error_perstep = combine_coeff_bitplanes_after_max_order(coefficient_error_perstep, coefficient_order);
        ...
        uint8_t * get_metadata(uint32_t& metadata_size) const {
            metadata_size =
                ...
                + get_size(coeff_sizes)                                     
                + get_size(combined_coeff_error_perstep)  
            ...
            *(p++) = static_cast<uint8_t>(negabinary);
            ...
            p += sizeof(uint16_t);
            serialize(coeff_sizes, p);
            serialize(combined_coeff_error_perstep, p);
