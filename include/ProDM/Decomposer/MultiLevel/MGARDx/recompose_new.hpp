#ifndef _MGARD_RECOMPOSE_NEW_HPP
#define _MGARD_RECOMPOSE_NEW_HPP
...
public:
	...
	void recompose(T * data_, const vector<size_t>& dims, size_t target_level, bool hierarchical=false, bool cubic=false, vector<size_t> strides=vector<size_t>()){
		data = data_;
		...
		if(dims.size() == 1){
			...
			for(int i=0; i<target_level; i++){
                if (cubic)
                    recompose_level_1D_cubic(data, level_dims[i+1][0], h);
                else
    				hierarchical ? recompose_level_1D_hierarhical_basis(data, n, h) : recompose_level_1D(data, level_dims[i+1][0], h);
				h >>= 1;
			}
		}
		else if(dims.size() == 2){
			for(int i=0; i<target_level; i++){
				...
				size_t n2 = dims[1];
                if (cubic)
                    recompose_level_2D_cubic(data, n1, n2, (T)h, strides[0]);
                else
            ...
            for(int i=0; i<target_level; i++){
				...
				size_t n3 = dims[2];
				if (cubic)
					recompose_level_3D_cubic(data, n1, n2, n3, (T)h, strides[0], strides[1]);
				else
...
private:
	...
	void recover_from_interpolant_difference_1D(const size_t begin, const size_t end, const size_t stride, T * buffer){
    ...
    void recover_from_interpolant_difference_1D_cubic(size_t n_coeff, const T * nodal_buffer, T * coeff_buffer){
    ...
    void recompose_level_1D_hierarhical_basis(T * data_pos, size_t n, T h, bool nodal_row=true){
        ...
        // T * coeff_buffer = data_buffer + n_nodal;
		// data_reverse_reorder_1D(data_pos, n_nodal, n_coeff, nodal_buffer, coeff_buffer);
        recover_from_interpolant_difference_1D(0, n-1, h, data_pos);
    ...
    void recompose_level_1D_cubic(T * data_pos, size_t n, T h, bool nodal_row=true)
    {
        ...
        T * nodal_buffer = data_buffer;
        T * coeff_buffer = data_buffer + n_nodal;
        recover_from_interpolant_difference_1D_cubic(n_coeff, nodal_buffer, coeff_buffer);
        data_reverse_reorder_1D(data_pos, n_nodal, n_coeff, nodal_buffer, coeff_buffer);
    }
	/* 
		2D recomposition
	*/
	...
	void recover_from_interpolant_difference_2D_vertical_cubic(T * data_pos, size_t n1, size_t n2, size_t stride){
		size_t n1_nodal = (n1 >> 1) + 1;
		size_t n1_coeff = n1 - n1_nodal;
		size_t n2_nodal = (n2 >> 1) + 1;
		size_t n2_coeff = n2 - n2_nodal;
	...
	void recover_from_interpolant_difference_2D(T * data_pos, size_t n1, size_t n2, size_t h, size_t stride){
		...
		for(int i=0; i<n2; i+=h){
			recover_from_interpolant_difference_1D(n1_begin, n1_end, stride_n1, cur_data_pos);
			cur_data_pos += h;
		}
		// compute vertical difference
		h = h >> 1;
		...
		size_t n2_begin = 0;
		size_t n2_end = n2 - 1;
		for(int i=0; i<n1; i+=h){
			recover_from_interpolant_difference_1D(n2_begin, n2_end, stride_n2, cur_data_pos);
			cur_data_pos += n2 * h;
		}
	}	
	void recover_from_interpolant_difference_2D_cubic(T * data_pos, size_t n1, size_t n2, size_t stride){
		size_t n1_nodal = (n1 >> 1) + 1;
		size_t n1_coeff = n1 - n1_nodal;
		size_t n2_nodal = (n2 >> 1) + 1;
		size_t n2_coeff = n2 - n2_nodal;
		...
		T * coeff_pos = data_pos + n2_nodal;
		for(int i=0; i<n1_nodal; i++){
			recover_from_interpolant_difference_1D_cubic(n2_coeff, nodal_pos, coeff_pos);
			nodal_pos += stride, coeff_pos += stride;
		}
		// compute vertical difference
		recover_from_interpolant_difference_2D_vertical_cubic(data_pos, n1, n2, stride);
	}
	...
	void recompose_level_2D(T * data_pos, size_t n1, size_t n2, T h, size_t stride){
		...
		data_reverse_reorder_2D_new(data_pos, data_buffer, n1, n2, stride);
	}
    ...
    void recompose_level_2D_hierarhical_basis(T * data_pos, size_t n1, size_t n2, T h, size_t stride){
        ...
        // size_t n2_coeff = n2 - n2_nodal;
		// data_reverse_reorder_2D_new(data_pos, data_buffer, n1, n2, stride);
        recover_from_interpolant_difference_2D(data_pos, n1, n2, h, stride);
    }
    // recompose n1/2 x n2/2 data into finer level (n1 x n2) with hierarchical basis (pure cubic interpolation)
    void recompose_level_2D_cubic(T * data_pos, size_t n1, size_t n2, T h, size_t stride){
        // cerr << "recompose, h = " << h << endl; 
        size_t n1_nodal = (n1 >> 1) + 1;
        size_t n1_coeff = n1 - n1_nodal;
        size_t n2_nodal = (n2 >> 1) + 1;
        size_t n2_coeff = n2 - n2_nodal;
        recover_from_interpolant_difference_2D_cubic(data_pos, n1, n2, stride);
        data_reverse_reorder_2D_new(data_pos, data_buffer, n1, n2, stride);
    }
    /*
    ...
    */
	...
	void recover_from_interpolant_difference_3D(T * data_pos, size_t n1, size_t n2, size_t n3, size_t h, size_t dim0_stride, size_t dim1_stride){
		...
		for(size_t i=0; i<n2; i+=h2x){
			...
			for(size_t j=0; j<n3; j+=h2x){
				recover_from_interpolant_difference_1D(n1_begin, n1_end, stride_n1, temp_data_pos);
				temp_data_pos += h2x;
			}
			cur_data_pos += n3 * h2x;
		}
		...
		for(size_t i=0; i<n1; i+=h){
			temp_data_pos = cur_data_pos;
			for(size_t j=0; j<n3; j+=h2x){
				recover_from_interpolant_difference_1D(n2_begin, n2_end, stride_n2, temp_data_pos);
				temp_data_pos += h2x;
			}
		...
		for(size_t i=0; i<n1; i+=h){
			...
			for(size_t j=0; j<n2; j+=h){
				recover_from_interpolant_difference_1D(n3_begin, n3_end, h, temp_data_pos);
				temp_data_pos += n3*h;
			}
			cur_data_pos += n2*n3*h;
		}
	}
	void recover_from_interpolant_difference_3D_cubic(T * data_pos, size_t n1, size_t n2, size_t n3, size_t dim0_stride, size_t dim1_stride){
		size_t n1_nodal = (n1 >> 1) + 1;
		size_t n1_coeff = n1 - n1_nodal;
		size_t n2_nodal = (n2 >> 1) + 1;
		size_t n2_coeff = n2 - n2_nodal;
		...
		T * cur_data_pos = data_pos;
 		for(int i=0; i<n1_nodal; i++){
 			recover_from_interpolant_difference_2D_cubic(cur_data_pos, n2, n3, dim1_stride);
 			cur_data_pos += dim0_stride;
 		}
 		// compute vertically
 		const T * nodal_pos = data_pos;
    ...
    void recompose_level_3D(T * data_pos, size_t n1, size_t n2, size_t n3, T h, size_t dim0_stride, size_t dim1_stride){
        ...
        data_reverse_reorder_3D_new(data_pos, data_buffer, n1, n2, n3, dim0_stride, dim1_stride);
    }
    ...
    void recompose_level_3D_hierarchical_basis(T * data_pos, size_t n1, size_t n2, size_t n3, T h, size_t dim0_stride, size_t dim1_stride){
        ...
        // size_t n3_nodal = (n3 >> 1) + 1;
		// data_reverse_reorder_3D_new(data_pos, data_buffer, n1, n2, n3, dim0_stride, dim1_stride);
        recover_from_interpolant_difference_3D(data_pos, n1, n2, n3, h, dim0_stride, dim1_stride);
    }
    void recompose_level_3D_cubic(T * data_pos, size_t n1, size_t n2, size_t n3, T h, size_t dim0_stride, size_t dim1_stride){
        size_t n1_nodal = (n1 >> 1) + 1;
        size_t n2_nodal = (n2 >> 1) + 1;
        size_t n3_nodal = (n3 >> 1) + 1;
        recover_from_interpolant_difference_3D_cubic(data_pos, n1, n2, n3, dim0_stride, dim1_stride);
        data_reverse_reorder_3D_new(data_pos, data_buffer, n1, n2, n3, dim0_stride, dim1_stride);
    }
};
