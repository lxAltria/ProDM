#ifndef PRODM_DECOMPOSER_MULTILEVEL_MGARDX_DECOMPOSE_INTERLEAVE_HPP
#define PRODM_DECOMPOSER_MULTILEVEL_MGARDX_DECOMPOSE_INTERLEAVE_HPP
...
private:
	...
	size_t compute_interpolant_difference_1D(const size_t begin, const size_t end, const size_t stride, T * data, T * buffer){
		...
		for(i=1; i+1<n; i+=2){
			...
			// if(abs(data[c] - (data[c - stride] + data[c + stride]) / 2) > abs_max){
			// 	abs_max = abs(data[c] - (data[c - stride] + data[c + stride]) / 2);
			// 	pos = (&data[c] - data_begin);
			// 	left = (&data[c - stride] - data_begin);
			// 	right = (&data[c + stride] - data_begin);
			// }
		...
		if(!(n&1)){
			...
			// if(abs(data[c] - data[c - stride]) > abs_max){
			// 	abs_max = abs(data[c] - data[c - stride]);
			// 	pos = (&data[c] - data_begin);
			// 	left = (&data[c - stride] - data_begin);
			// 	right = -1;
			// }
	...
	size_t compute_interpolant_difference_1D_cubic(const size_t begin, const size_t end, const size_t stride, T * data, T * buffer){
		...
		data[c] = interp_quad_1(data[c - stride], data[c + stride], data[c + stride3x]);
		for(i=3; i+3<n; i+=2){
			c = begin + i * stride;
			// buffer[c] -= (buffer[c - stride] + buffer[c + stride]) / 2;
			// std::cout << (&data[c] - data_begin) << " -= (" << (&data[c - stride] - data_begin) << " + " << (&data[c + stride] - data_begin) << ")/2" << std::endl;
			buffer[count++] = data[c] - interp_cubic(data[c - stride3x], data[c - stride], data[c + stride], data[c + stride3x]);
			data[c] = interp_cubic(data[c - stride3x], data[c - stride], data[c + stride], data[c + stride3x]);
		}
		c = begin + i * stride;
		buffer[count++] = data[c] - interp_quad_2(data[c - stride3x], data[c - stride], data[c + stride]);
		data[c] = interp_quad_2(data[c - stride3x], data[c - stride], data[c + stride]);
		if(!(n&1)){
			c = begin + (n - 1) * stride;
			// std::cout << (&data[c] - data_begin) << " -= " << (&data[c - stride] - data_begin) << std::endl;
			// buffer[c] -= buffer[c - stride];
			buffer[count++] = data[c] - interp_quad_3(data[c - stride5x], data[c - stride3x], data[c - stride]);
			data[c] = interp_quad_3(data[c - stride5x], data[c - stride3x], data[c - stride]);
		}
		return count;
	}
	...
	size_t compute_interpolant_difference_1D_diff_direct(const size_t begin, const size_t end, const size_t stride, const size_t interp_stride, bool last_even, T * data, T * buffer){
		...
		if(!last_even){
			for(i=0; i<n; i++){
				...
				// if(abs(data[c] - (data[c - interp_stride] + data[c + interp_stride]) / 2) > abs_max){
				// 	abs_max = abs(data[c] - (data[c - interp_stride] + data[c + interp_stride]) / 2);
				// 	pos = (&data[c] - data_begin);
				// 	left = (&data[c - interp_stride] - data_begin);
				// 	right = (&data[c + interp_stride] - data_begin);
				// }
			...
			for(i=0; i<n; i++){
				...
				// if(abs(data[c] - data[c - interp_stride]) > abs_max){
				// 	abs_max = abs(data[c] - data[c - interp_stride]);
				// 	pos = (&data[c] - data_begin);
				// 	left = (&data[c - interp_stride] - data_begin);
				// 	right = -1;
				// }
		...
		switch (interp_type)
		{
			case FIRST_LINE:
			{
				for(i=0; i<n; i++){
					...
					data[c] = interp_quad_1(data[c - interp_stride], data[c + interp_stride], data[c + interp_stride3x]);
				}
				break;
			}
			case NORMAL_LINE:
			{
				for(i=0; i<n; i++){
					...
					data[c] = interp_cubic(data[c - interp_stride3x], data[c - interp_stride], data[c + interp_stride], data[c + interp_stride3x]);
				}
				break;
			}
			case LAST_LINE:
			{
				for(i=0; i<n; i++){
					...
					data[c] = interp_quad_2(data[c - interp_stride3x], data[c - interp_stride], data[c + interp_stride]);
				}
				break;
			}
			case LAST_EVEN_LINE:
			{
				for(i=0; i<n; i++){
					...
					data[c] = interp_quad_3(data[c - interp_stride5x], data[c - interp_stride3x], data[c - interp_stride]);
				}
				break;
			}
			default:
	...
	size_t compute_interpolant_difference_2D_cubic(T * data_pos, size_t n1, size_t n2, size_t h, size_t current_level){
		...
		cur_buffer_pos = level_buffers[((current_level - 1) * 2) + 1].data();
		for(size_t i=0; i<n1; i+=h2x){
			count = compute_interpolant_difference_1D_cubic(n2_begin, n2_end, stride_n2, cur_data_pos, cur_buffer_pos);
			cur_data_pos += n2 * h2x;
			cur_buffer_pos += count;
			count_2D += count;
		}
		...
		cur_data_pos = data_pos + stride_n1;
		cur_buffer_pos = level_buffers[((current_level - 1) * 2) + 2].data();
		count = compute_interpolant_difference_1D_cubic_diff_direct(n2_begin, n2_end, h, stride_n1, FIRST_LINE, cur_data_pos, cur_buffer_pos);
		cur_data_pos += 2 * stride_n1;
		cur_buffer_pos += count;
		count_2D += count;
		for(size_t i=3; i+3<new_n1; i+=2){
			count = compute_interpolant_difference_1D_cubic_diff_direct(n2_begin, n2_end, h, stride_n1, NORMAL_LINE, cur_data_pos, cur_buffer_pos);
			cur_data_pos += 2 * stride_n1;
			cur_buffer_pos += count;
			count_2D += count;
		}
		count = compute_interpolant_difference_1D_cubic_diff_direct(n2_begin, n2_end, h, stride_n1, LAST_LINE, cur_data_pos, cur_buffer_pos);
		cur_data_pos += 2 * stride_n1;
		cur_buffer_pos += count;
		count_2D += count;
		if(!(new_n1 & 1)){
			count = compute_interpolant_difference_1D_cubic_diff_direct(n2_begin, n2_end, h, stride_n1, LAST_EVEN_LINE, cur_data_pos, cur_buffer_pos);
			count_2D += count;
		}
	...
	size_t compute_interpolant_difference_3D(T * data_pos, size_t n1, size_t n2, size_t n3, size_t h, size_t current_level){
		...
		for(size_t i=0; i<n1; i+=h2x){
			...
			for(size_t j=0; j<n2; j+=h2x){
				count = compute_interpolant_difference_1D(n3_begin, n3_end, h, temp_data_pos, cur_buffer_pos);
				temp_data_pos += n3 * h2x;
				cur_buffer_pos += count;
				count_3D += count;
			}
			cur_data_pos += n2 * n3 * h2x;
		}
	...
	size_t compute_interpolant_difference_3D_cubic(T * data_pos, size_t n1, size_t n2, size_t n3, size_t h, size_t current_level){
		...
		for(size_t i=0; i<n1; i+=h2x){
			temp_data_pos = cur_data_pos;
			for(size_t j=0; j<n2; j+=h2x){
				count = compute_interpolant_difference_1D_cubic(n3_begin, n3_end, h, temp_data_pos, cur_buffer_pos);
				temp_data_pos += n3 * h2x;
				cur_buffer_pos += count;
				count_3D += count;
			}
		...
		for(size_t i=0; i<n1; i+=h2x){
			temp_data_pos = cur_data_pos;
			// First line
			count = compute_interpolant_difference_1D_cubic_diff_direct(n3_begin, n3_end, h, stride_n2, FIRST_LINE, temp_data_pos, cur_buffer_pos);
			temp_data_pos += n3 * h2x;
			cur_buffer_pos += count;
			count_3D += count;
			// Normal lines
			for(size_t j=3; j+3<new_n2; j+=2){
				count = compute_interpolant_difference_1D_cubic_diff_direct(n3_begin, n3_end, h, stride_n2, NORMAL_LINE, temp_data_pos, cur_buffer_pos);
				temp_data_pos += n3 * h2x;
				cur_buffer_pos += count;
				count_3D += count;
			}
			// Last line or Second Last line
			count = compute_interpolant_difference_1D_cubic_diff_direct(n3_begin, n3_end, h, stride_n2, LAST_LINE, temp_data_pos, cur_buffer_pos);
			temp_data_pos += n3 * h2x;
			cur_buffer_pos += count;
			count_3D += count;
			// Last even line if exists
			if(!(new_n2 & 1)){
				count = compute_interpolant_difference_1D_cubic_diff_direct(n3_begin, n3_end, h, stride_n2, LAST_EVEN_LINE, temp_data_pos, cur_buffer_pos);
				cur_buffer_pos += count;
				count_3D += count;
			}
		...
		// First plane
		for(size_t j=0; j<n2; j+=h){
			count = compute_interpolant_difference_1D_cubic_diff_direct(n3_begin, n3_end, h, stride_n1, FIRST_LINE, temp_data_pos, cur_buffer_pos);
			temp_data_pos += stride_n2;
			cur_buffer_pos += count;
			count_3D += count;
		}
		...
		for(size_t i=3; i+3<new_n1; i+=2){
			temp_data_pos = cur_data_pos;
			for(size_t j=0; j<n2; j+=h){
				count = compute_interpolant_difference_1D_cubic_diff_direct(n3_begin, n3_end, h, stride_n1, NORMAL_LINE, temp_data_pos, cur_buffer_pos);
				temp_data_pos += stride_n2;
				cur_buffer_pos += count;
				count_3D += count;
			}
		...
		// Last plane or Second Last plane
		temp_data_pos = cur_data_pos;
		for(size_t j=0; j<n2; j+=h){
			count = compute_interpolant_difference_1D_cubic_diff_direct(n3_begin, n3_end, h, stride_n1, LAST_LINE, temp_data_pos, cur_buffer_pos);
			temp_data_pos += stride_n2;
			cur_buffer_pos += count;
			count_3D += count;
		}
		cur_data_pos += 2 * stride_n1;
		// Last even plane if exists
		if(!(new_n1 & 1)){
			temp_data_pos = cur_data_pos;
			for(size_t j=0; j<n2; j+=h){
				count = compute_interpolant_difference_1D_cubic_diff_direct(n3_begin, n3_end, h, stride_n1, LAST_EVEN_LINE, temp_data_pos, cur_buffer_pos);
				temp_data_pos += stride_n2;
				cur_buffer_pos += count;
				count_3D += count;
			}
