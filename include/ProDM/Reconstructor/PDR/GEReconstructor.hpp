...
namespace ProDM::PDR {
    ...
    {
    public:
        GEReconstructor(Approximator approximator, Encoder encoder, Compressor compressor, SizeInterpreter interpreter, Retriever retriever)
        ...
        bool load_weight()
        {
            ...
            if (num_bytes < sizeof(unsigned int) + sizeof(size_t) + sizeof(uint32_t)){
                fclose(file);
                return false;
            }
            rewind(file);
        ...
        bool reconstruct(const std::vector<uint8_t> &prev_level_num_bitplanes, bool progressive = true)
        {
            ...
            if (level_num_bitplanes[i] - prev_level_num_bitplanes[i] > 0)
            {
                if (mask.empty()){
                    ...
                    // filtered_timer.end();
                    // filtered_timer.print("Filtering weight");
                    // Timer decompressor_timer;
                    // decompressor_timer.start();
                    compressor.decompress_level(level_components[i], level_sizes[i], prev_level_num_bitplanes[i], level_num_bitplanes[i] - prev_level_num_bitplanes[i], stopping_indices[i]);
                    // decompressor_timer.end();
