...
void usage(char* cmd) {
    std::cout << "PSZ3-delta usage: " << std::endl <<
                  "mpirun -n #num_cores para_PSZ3-delta_refactor data_file output_dict num_snapshot num_dim dim0 .. dimn -[dataType: f/d]"
                  << std::endl
                  << "example: " << std::endl <<
                  "mpirun -n 2 para_PSZ3-delta_refactor density refactor/Density_refactored 18 3 256 384 384 -d" << std::endl;
}
...
int main(int argc, char ** argv){
    ...
    // output_dict = output_dict + "/" + to_string(idx) + "_" + to_string(idy) + "_" + to_string(idz) + "/";

    filename = filename + to_string(rank) + ".d64";
    output_dict = output_dict + "/" + to_string(rank) + "/";
