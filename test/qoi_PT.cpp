...
float * Vz_dec = NULL;
float * PT_ori = NULL;
std::vector<float> error_PT;
std::vector<float> error_est_PT;
int iter = 0;
...
template<class T>
std::vector<size_t> retrieve_PT_Dummy(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, const std::vector<unsigned char>& mask, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size){
    int max_iter = 10;
...
template<class T>
std::vector<size_t> retrieve_PT_SZ3(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, const std::vector<unsigned char>& mask, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size){
    int max_iter = 10;
	bool tolerance_met = false;
	int n_variable = ebs.size();
	std::vector<std::vector<T>> reconstructed_vars(n_variable, std::vector<T>(num_elements));
...
template<class T>
std::vector<size_t> retrieve_PT_PMGARD(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, const std::vector<unsigned char>& mask, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size){
    int max_iter = 10;
	bool tolerance_met = false;
	int n_variable = ebs.size();
	std::vector<std::vector<T>> reconstructed_vars(n_variable, std::vector<T>(num_elements));
...
template<class T>
std::vector<size_t> retrieve_PT_GE(std::string rdata_file_prefix, T tau, std::vector<T> ebs, size_t num_elements, const std::vector<unsigned char>& mask, int weighted, T & max_act_error, T & max_est_error, size_t & weight_file_size){
    int max_iter = 10;
    bool tolerance_met = false;
    int n_variable = ebs.size();
    std::cout << "n_variable = " << n_variable << std::endl; 
...
int main(int argc, char ** argv){
	...
	{
	case Dummy:
		total_retrieved_size = retrieve_PT_Dummy(rdata_file_prefix, tau, ebs, num_elements, mask, weighted, max_act_error, max_est_error, weight_file_size);
		break;
	case SZ3:
		total_retrieved_size = retrieve_PT_SZ3(rdata_file_prefix, tau, ebs, num_elements, mask, weighted, max_act_error, max_est_error, weight_file_size);
		break;
	case PMGARD:
		total_retrieved_size = retrieve_PT_PMGARD(rdata_file_prefix, tau, ebs, num_elements, mask, weighted, max_act_error, max_est_error, weight_file_size);
		break;
    case GE:
		total_retrieved_size = retrieve_PT_GE(rdata_file_prefix, tau, ebs, num_elements, mask, weighted, max_act_error, max_est_error, weight_file_size);
		break;
	default:
		break;
	}
