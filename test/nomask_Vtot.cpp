...
bool halfing_error_V_TOT_uniform(const T * Vx, const T * Vy, const T * Vz, size_t n, const std::vector<unsigned char>& mask, const double tau, std::vector<double>& ebs, std::vector<std::vector<int>> weights){
	...
	for(int i=0; i<n; i++){
		...
		double e_V_TOT_2 = 0;
		if(mask[i]) e_V_TOT_2 = compute_bound_x_square(Vx[i], eb_Vx / std::pow(2.0, weights[0][i])) + compute_bound_x_square(Vy[i], eb_Vy / std::pow(2.0, weights[1][i])) + compute_bound_x_square(Vz[i], eb_Vz / std::pow(2.0, weights[2][i]));
	...
	if(max_value > tau){
		...
		while(estimate_error > tau){
			...
			eb_Vy = eb_Vy / 1.5;
			eb_Vz = eb_Vz / 1.5;		        		
			double e_V_TOT_2 = compute_bound_x_square(Vx[i], eb_Vx / std::pow(2.0, weights[0][i])) + compute_bound_x_square(Vy[i], eb_Vy / std::pow(2.0, weights[1][i])) + compute_bound_x_square(Vz[i], eb_Vz / std::pow(2.0, weights[2][i]));
...
int main(int argc, char ** argv){
    ...
    while((!tolerance_met) && (iter < max_iter)){
    	iter ++;
	    for(int i=0; i<n_variable; i++){
	        auto reconstructed_data = reconstructors[i].progressive_reconstruct(ebs[i] / std::pow(2.0, 4), -1);
	    ...
	    int x_pos = MGARD::print_statistics(Vx_ori.data(), Vx_dec, num_elements);
	    int y_pos = MGARD::print_statistics(Vy_ori.data(), Vy_dec, num_elements);
	    int z_pos = MGARD::print_statistics(Vz_ori.data(), Vz_dec, num_elements);
		std::cout << "Weight at x_pos = " << weights[0][x_pos] << ", Weight at y_pos = " << weights[1][y_pos] << ", Weight at z_pos = " << weights[2][z_pos] << std::endl;
