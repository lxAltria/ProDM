...
void evaluate(const vector<T>& data, const vector<double>& tolerance, Reconstructor reconstructor){
    ...
    for(int i=0; i<tolerance.size(); i++){
        ...
        cout << "Bitrate = " << (reconstructor.get_retrieved_size() * sizeof(T) * 1.0) / data.size() << std::endl;
...
void test_XORBitplaneReconstructor(string filename, string refactored_path, const vector<double>& tolerance){
    ...
    auto interpreter = MDR::SignExcludeGreedyBasedSizeInterpreter<MDR::MaxErrorEstimatorHB<T>>(estimator);
    // auto interpreter = MDR::SignExcludeDPBasedSizeInterpreter<MDR::MaxErrorEstimatorHB<T>>(estimator);
    auto reconstructor = MDR::BitplaneReconstructor<T, MDR::MGARDHierarchicalDecomposer<T>, MDR::DirectInterleaver<T>, MDR::XORNegaBinaryBPEncoder<T, T_stream>, MDR::AdaptiveLevelCompressor, MDR::SignExcludeGreedyBasedSizeInterpreter<MDR::MaxErrorEstimatorHB<T>>, MDR::MaxErrorEstimatorHB<T>, MDR::ConcatLevelFileRetriever>(decomposer, interleaver, encoder, compressor, interpreter, retriever);
    cout << "loading metadata" << endl;
    reconstructor.load_metadata();
...
void test_MDR(string filename, string refactored_path, const vector<double>& tolerance){
    ...
    auto interpreter = MDR::SignExcludeGreedyBasedSizeInterpreter<MDR::MaxErrorEstimatorHB<T>>(estimator);
    // auto interpreter = MDR::SignExcludeDPBasedSizeInterpreter<MDR::MaxErrorEstimatorHB<T>>(estimator);
    auto reconstructor = MDR::ComposedReconstructor<T, MDR::MGARDHierarchicalDecomposer<T>, MDR::DirectInterleaver<T>, MDR::NegaBinaryBPEncoder<T, T_stream>, MDR::AdaptiveLevelCompressor, MDR::SignExcludeGreedyBasedSizeInterpreter<MDR::MaxErrorEstimatorHB<T>>, MDR::MaxErrorEstimatorHB<T>, MDR::ConcatLevelFileRetriever>(decomposer, interleaver, encoder, compressor, interpreter, retriever);
    cout << "loading metadata" << endl;
    reconstructor.load_metadata();
...
int main(int argc, char ** argv){
    ...
    switch(option){
        ...
        case 1:
        {
            test_XORBitplaneReconstructor<T, T_stream>(filename, refactored_path, tolerance);
            break;
        }
        case 2:
        {
            test_MDR<T, T_stream>(filename, refactored_path, tolerance);
            break;
        }
        case 3:
        {
